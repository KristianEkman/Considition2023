...
namespace Considition2023_Cs
{
    internal class Scoring
    {
        ...
        public static double FastScore(SubmitSolution solution, MapData mapEntity, GeneralData generalData)
        {

            Dictionary<string, StoreLocationScoring> locations = new();
            ...
            foreach (KeyValuePair<string, StoreLocationScoring> kvp in locations)
            {
                ...
                kvp.Value.GramCo2Savings = sales * (generalData.ClassicUnitData.Co2PerUnitInGrams - generalData.RefillUnitData.Co2PerUnitInGrams);
                kgCo2Savings += kvp.Value.GramCo2Savings / 1000;
                if (kvp.Value.GramCo2Savings > 0)
                {
                    kvp.Value.IsCo2Saving = true;
                }
                ...
                kvp.Value.Earnings = kvp.Value.Revenue - kvp.Value.LeasingCost;
                totalLeasingCost += kvp.Value.LeasingCost;

                totalFreestyle3100Count += kvp.Value.Freestyle3100Count;
                totalFreestyle9100Count += kvp.Value.Freestyle9100Count;

                totalFootfall += kvp.Value.Footfall;
            }

            //Just some rounding for nice whole numbers
            totalRevenue = Math.Round(totalRevenue, 0);
            kgCo2Savings = Math.Round(
                kgCo2Savings
                - totalFreestyle3100Count * generalData.Freestyle3100Data.StaticCo2 / 1000
                - totalFreestyle9100Count * generalData.Freestyle9100Data.StaticCo2 / 1000
                , 0);
            ...
            var total = Math.Round(
                (kgCo2Savings * generalData.Co2PricePerKiloInSek + earnings) * (1 + totalFootfall),
                0
            );
